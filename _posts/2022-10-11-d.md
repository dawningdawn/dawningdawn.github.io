...
* It is made of a stack of N=6 identical layers, each layer having 2 sub-layers.
    ...
    * 2nd sub-layer: A simple, position-wise fully connected feed-forward network*.
      ...
      <small>*Position-wise feed-forward layer: A type of feedforward layer consisting of two dense layers that applies to the last dimension, which means the same dense layers are used for each position item in the sequence, so called position-wise.</small>
